...
Type=oneshot
RemainAfterExit=no
dnl ExecStartPre=-/usr/bin/docker stop --time=DOCKER_STOP_TIMEOUT DOCKER_NAME
dnl ExecStartPre=-/usr/bin/docker rm DOCKER_NAME
...
dnl                          --name DOCKER_NAME \
                          --rm \
                          --volume DOCKER_VOLUME \
                          --entrypoint=/bin/bash \
                          DOCKER_IMAGE \
                            -c 'mysqldump MYSQL_BACKUP_ARGS --result-file=MYSQL_BACKUP_FILE.tmp --user=MYSQL_USER --password=MYSQL_PASS --port=MYSQL_PORT --host=MYSQL_HOST --databases MYSQL_DATABASE && mv -f MYSQL_BACKUP_FILE.tmp MYSQL_BACKUP_FILE'
