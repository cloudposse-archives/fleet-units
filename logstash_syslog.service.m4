...
define(DNS_SERVICE_ID, {{{{syslog}}}})dnl
...
define(ELASTICSEARCH_INDEX, {{syslog}})dnl
...
ExecStart=/usr/bin/docker run \
                          ...
                          -e "SERVICE_514_NAME=DNS_SERVICE_NAME" \
                          -e "SERVICE_514_ID=DNS_SERVICE_ID" \
                          --entrypoint /bin/sh \
                          DOCKER_IMAGE \
                          /logstash/bin/logstash agent \
                          --config /logstash/logstash-syslog.conf \
                          -e 'output { {{elasticsearch_}}ELASTICSEARCH_SCHEME { host => "ELASTICSEARCH_HOST" index => "ELASTICSEARCH_INDEX-%{+YYYY.MM.dd}" port => ELASTICSEARCH_PORT } }' 
